                ...
                while (index >= 0 && this.requests[index].startTime > nextDateOffset) {
                    ...
                    if (request.response) {
                        switch (request.response.status) {
                            case 'error': 
                                currentSet.requests.errored++;
                                break;

                            case 'timeout':
                                currentSet.requests.timeout++;
                                break;

                            case 'success':
                                currentSet.requests.success++;
                            ...
                            case 'failed':
                                currentSet.requests.failed++;
                                break;

                            case 'waiting':
                                currentSet.requests.waiting++;
                                break;
                        }
                    }
                    else currentSet.requests.waiting++;
                ...
                Object.keys(currentSet.timings).forEach((key) => {
                    ...
                    if (set.length) {
                        ...
                        [50, 90, 95, 98, 99, 99.5, 99.9].forEach((percentile) => {
                         ...
                         currentSet.average[key] = null;

                         [50, 90, 95, 98, 99, 99.5, 99.9].forEach((percentile) => {
                            currentSet.percentiles[key][percentile] = null;
                        });
                    }
                });
